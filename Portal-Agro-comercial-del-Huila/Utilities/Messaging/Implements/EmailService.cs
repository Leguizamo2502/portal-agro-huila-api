...
namespace Utilities.Messaging.Implements
{
    public class EmailService : ISendCode
    {
        ...
        public async Task SendVerificationCodeEmail(string emailReceptor, string verificationCode)
        {
            var emailEmisor = _config["CONFIG_EMAIL:EMAIL"]!;
            var password = _config["CONFIG_EMAIL:PASSWORD"];
            var host = _config["CONFIG_EMAIL:HOST"];
            ...
            string asunto = "Verificación de correo - Portal Agro Comercial del Huila";
            string cuerpoHtml = $@"
                <!DOCTYPE html>
                <html lang='es'>
                <head><meta charset='UTF-8'><title>Verificación de correo</title></head>
                <body style='font-family: Arial, sans-serif; background: #f4f4f4; padding: 40px;'>
                    <div style='max-width: 600px; margin: auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);'>
                        <h2 style='color: #4a5568;'>Confirma tu correo electrónico</h2>
                        <p style='font-size: 16px; color: #2d3748;'>Gracias por registrarte en el Portal Agro Comercial del Huila.</p>
                        <p style='font-size: 16px; color: #2d3748;'>Tu código de verificación es:</p>
                        <div style='font-size: 28px; font-weight: bold; color: #667eea; margin: 20px 0;'>{verificationCode}</div>
                        <p style='font-size: 14px; color: #718096;'>Este código vence en 10 minutos.</p>
                        <br>
                        <p style='font-size: 12px; color: #a0aec0;'>Portal Agro-Comercial del Huila © 2025</p>
                    </div>
                </body>
