...
namespace Data.Interfaces.Implements.Notifications
{
    ...
    {
        Task<IEnumerable<Notification>> GetUnreadAsync(int userId, int take = 20, CancellationToken ct = default);
        Task<int> CountUnreadAsync(int userId, CancellationToken ct = default);

        Task<(IEnumerable<Notification> Items, int Total)> GetHistoryAsync(
