...
namespace Business.Interfaces.Implements.Notification
{
    public interface INotificationService
    {
        ...
        Task<IEnumerable<NotificationListItemDto>> GetUnreadAsync(int userId, int take = 20, CancellationToken ct = default);
        Task<int> CountUnreadAsync(int userId, CancellationToken ct = default);
        Task<(IEnumerable<NotificationListItemDto> Items, int Total)> GetHistoryAsync(int userId, int page, int pageSize, CancellationToken ct = default);
